...
- VPC multi-AZ; EKS com node group autoscaling; HPA nos pods; Aurora Postgres multi-AZ; Batch com CE spot/on-demand; ALB/Ingress multi-AZ; buckets S3 com KMS. Pode incluir CloudFront + WAF para borda.

## Por que EKS para API/Front?
- Padrao Kubernetes: facilita IRSA, ingress controller, HPA, observabilidade (Prometheus/Grafana), e blue/green/Canary via ingress/Argo Rollouts. Se front for static, pode ir para S3+CloudFront; se SSR, pode rodar em EKS/ECS.
...
- Secrets em Secrets Manager/SSM; IRSA para pods; SGs restritos (ALB -> pods; pods -> RDS); KMS para S3; TLS em transito; WAF/CloudFront opcional; CloudTrail/Config habilitados; roles least privilege.
