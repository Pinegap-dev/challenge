...
3) Planejar DNS/ACM/WAF (se usar CloudFront/ALB).

## Visao geral
- IaC modular em `entregas/challenge-04/`:
...
1) Ajustar variaveis em `main.tf`:
   - `db_password`, `region`, `environment`, `batch_job_image`, tamanhos (RDS classe, node group).
2) Executar:
   ```bash
